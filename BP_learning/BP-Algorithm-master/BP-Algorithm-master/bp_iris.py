...
def forward_propagate(network, inputs):#计算network中的节点值
    """前向传播计算
    :param network: 神经网络
    :param inputs: 一个样本数据
    :return: 前向传播计算的结果
    ...
    for layer in network:  # 循环计算每一层
        ...
        inputs = new_inputs  #返回输出层三个神经元的输出值
    return inputs


def backward_propagate_error(network, actual_label): #计算network中的误差项
    """误差进行反向传播
    :param network: 神经网络
...
def initialize_network(n_inputs, n_hidden, n_outputs):#计算network中的每个节点的权重
...
def train(train_data, l_rate, epochs, n_hidden, val_data):
    ...
    # 获取分类的总类别数
    n_outputs = len(set([row[-1] for row in train_data]))     #遍历每一行，set将去除重复的元素
    # 初始化网络
    network = initialize_network(n_inputs, n_hidden, n_outputs) #network 其实就是储存每层节点权值的二维矩阵
    ...
    for epoch in range(epochs):  # 训练epochs个回合
        for row in train_data:
            ...
            # 更新参数
            update_parameters(network, row, l_rate)#把预测正确的数量除以总数量作为 每次epoch精确率
...
def validation(network, val_data):
    ...
    :return: 模型在验证集上的准确率
    """
    # 获取预测类标，把输出层的三个神经元输出最大的值作为预测值
    predicted_label = []
    for row in val_data:
        prediction = predict(network, row)   #返回输出层最大值的索引
        predicted_label.append(prediction)   #记录一个epoch的所有索引
    # 获取真实类标
...
def accuracy_calculation(actual_label, predicted_label):
    ...
    for i in range(len(actual_label)):
        if actual_label[i] == predicted_label[i]:
            correct_count += 1
    return correct_count / float(len(actual_label)) * 100.0   #把预测正确的数量除以总数量作为 精确率
...
if __name__ == "__main__":
    ...
    n_train_data = 100  # 训练集的大小（总共150条数据，训练集130条，验证集20条）
